...
function PinDetail({user}) {
  ...
  const fetchPinDetails = ()=>{
    ...
    if(query){
      client.fetch(query)
      .then((data)=>{
        setPinDetails(data[0]) // get pin current

        if(data[0]){
          query = pinDetailMorePinQuery(data[0])

          client.fetch(query).then((res) =>setPins(res)) // get more pin relate
        }
      })
    }
  }
  ...
  return (
    ...
    >
      <div className='flex justify-center items-center md:items-start flex-initial'>
        <img src={pinDetails?.image && urlFor(pinDetails.image).url()} alt="pin-detail"
