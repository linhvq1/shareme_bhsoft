...
export const feedQuery = (sortBy) => {
  let query = `*[_type == "pin" && publicPin == true] | order(${
    sortBy ? sortBy : "_createdAt"
  } ${sortBy === "title" && sortBy ? "asc" : "desc"}) {
    image{
      asset->{
        url
      }
...
export const userCreatedPinsQuery = (userId, sortBy, owner) => {
  const query = `*[ _type == 'pin' && userId == '${userId}'  ${
    owner ? "" : "&& publicPin == true"
  }] | order(${sortBy} ${sortBy === "title" ? "asc" : "desc"}){
    image{
      asset->{
        url
      }
...
export const userSavedPinsQuery = (userId, sortBy, owner) => {
  const query = `*[_type == 'pin'  ${
    owner ? "" : "&& publicPin == true"
  } && '${userId}' in save[].userId ] | order(${sortBy} ${
